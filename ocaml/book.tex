...
{\tt \b} & Matches a word boundary. & {\tt a\b} matches the second a in ``aa'' but not the first. \\ \hline
{\tt .} & Matches any character except newline & {\tt .*} will match any line of a string \\ \hline
{\tt \^} & Matches the beginning of a line or string \\ {\tt \^a} will match the first a in ``aa'' but not the second.\hline
...
{\tt \\|} & Infix operator. Matches either the expression before the bar or the expression after the bar, but not both at once. This is distinct from character classes in that it can operate on larger grouped expressions. \\ {\tt the\\|a} matches ``a'' or ``the''. \hline
{\tt [...]} & Defines a character class. It matches exactly one character, of any within the class. You can define a range using ``-''. To include a ``]'', make it the first character within the class. To include a ``-'', make it the first or last character of the class. Negate the class using \^. & {\tt [0-9a-zA-Z] matches any single alphanumeric character. \\ \hline
{\tt \\(...\\) & Groups a subexpression. & {\tt Jer(ry\\|i)} matches both ``Jerry'' and ``Jeri''. \\ \hline
{\tt \\1} & Matches the text matched by the first subexpression. (\\2 matches the second, and so on up to \\9) & {\tt (the) \\1} matches ``the the''. \\ \hline
{\tt \\} & Matches a special character, i.e., one of {\tt \$\^.*+?[]}. & {\tt \\\$[0-9]*} matches any even dollar amount. \\ \hline
\label{tab:regex}
\end{tabular}
...
\end{verbatim}
\afterverb
{\tt Str.string_match} is a function which takes a regular expression, a string, and an int, and determines whether or not a substring starting at the position specified\footnote{Strings are indexed from 0!} matches the regular expression. Note that this doesn't test the entire string, only searches for the (longest) substring starting at that position:
\beforeverb
\begin{verbatim}
# let s2 = "baaa";;
# let s3 = "aaab";;
